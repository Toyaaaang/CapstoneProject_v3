...
export type PendingCreditReturn = {
    id: number;
    mrt_no: string;
    returned_by: string;
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const handleAction = async (status: "approved" | "rejected") => {
        try {
          await axios.post(`/warehouse-admin/credit/${row.original.id}/${status}/`);
          toast.success(`Credit return ${status}`);
          refreshData();
        } catch (err) {
          console.error(err);
          toast.error(`Failed to ${status} credit return`);
        }
      };
